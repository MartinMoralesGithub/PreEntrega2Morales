    ...
    if(sumarCantidades() === 0){
        ...
        return(
            ...
            <div className={`${cart.length > 3 ? 'px-16 py-8 grid grid-cols-4 gap-[30px]' : 'px-16 pt-2 pb-8 flex gap-[30px] justify-center'}`}>
                {cart.map(item => (
                    <div className={`${cart.length > 3 ? 'my-auto p-12 rounded-lg text-center shadow-xl h-[32rem] ' : 'my-auto p-12 rounded-lg text-center shadow-xl h-[32rem] w-1/4'}`} key={item.id}>    
                        <h2 className="text-xl font-bold mb-2">{item.nombre}</h2>
                        <p className="text-gray-600">{item.descripcion}</p>
                        <p className="text-gray-600">Precio: ${item.precio}</p>
                        <img src={item.imagen} className="mt-4 mx-auto" />
                        <p className="mt-4 mx-auto font-medium" >Cantidad: {item.counter}</p>
                        <button 
            ...
            <div>
                ...
                <h1 className="text-xl font-semibold mb-2 text-center mt-12">Total: ${sumarPrecios()}</h1>
                <Link 
                    to='/checkout'
                    className="bg-[#FF6C4D] hover:bg-[#ff8b74] text-white font-bold py-4 px-4 rounded mt-4 transition-colors duration-300 flex justify-center items-center"
                    >Terminar compra
                </Link>
            </div>
